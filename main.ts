import { Plugin, Notice, ItemView, WorkspaceLeaf, Setting, MarkdownRenderer, TFile, loadPdfJs } from 'obsidian';
...
export default class PdfPlugin extends Plugin {
    ...
    async generatePdfData(file: TFile, showLineNumbers: boolean = false): Promise<ArrayBuffer | null> {
        ...
        const activeFont = isBuiltIn ? this.settings.fontFamily : 'custom-font';
        ...
        if (this.settings.fontFamily === 'custom' && this.settings.customFontPath) {
            ...
            if (fontFile instanceof TFile && (fontFile.extension === 'ttf' || fontFile.extension === 'otf')) {
                try {
                    ...
                    doc.addFont(fontName, activeFont, 'normal');
                    doc.addFont(fontName, activeFont, 'bold');
                    doc.addFont(fontName, activeFont, 'italic');
                    doc.addFont(fontName, activeFont, 'bolditalic');
                } catch (e) {
    ...
    async exportToPdf() {
        ...
        try {
            ...
            const pdfPath = (file.parent ? file.parent.path : "") + "/" + file.basename + ".pdf";
...
class PdfSidebarView extends ItemView {
    ...
    async display() {
        ...
        if (this.plugin.settings.fontFamily === 'custom') {
            new Setting(container).setName("Font Path (.ttf)").setDesc("Path to the .ttf file in your vault").addText(t => {
                t.setPlaceholder("e.g. Resources/Fonts/Inter.ttf")
